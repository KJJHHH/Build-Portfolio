...
class feature_engineer:
    ...
    def transform_standard_winsor(self, X_train, X_test = None):
        ...
        if self.winsor == True:
            ...
            X_train = sefl.to_final_index(X_train, train_final_index)
            if type(X_test) == pd.core.frame.DataFrame:
                X_test = X_test.groupby(['mdate']).apply(winsorise)
                X_test = to_final_index(X_test, test_final_index)
    ...
    def transform_poly_selected_pearsonr(X_train):
        # 1. poly features
        ...
        for i in range(X_poly.shape[1]):
            print(i)
            corr, _ = pearsonr(X_poly[:, i], y_train)
            if corr >= 0.005:
