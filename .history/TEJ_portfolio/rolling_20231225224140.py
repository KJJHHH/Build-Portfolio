...
class Rolling_Prediction:
    ...
    def feature_engineering(self, X, y_train = None):
        # Check self.feature_eng
        if y_train != None:
            X_scale_winsor = self.feature_eng.transform_standard_winsor(X, scale_method='Standard', winsor=True)
            X_poly_selected_corr, sele = self.feature_eng.transform_poly_selected_pearsonr(X_scale_winsor, y_train)
            X_pca = self.feature_eng.transform_pca(X_poly_selected_corr, var = 100)
            X_selected_bic = self.feature_eng.selected_bic(X_pca, y_train)
        else:
            X_scale_winsor = self.feature_eng.transform_standard_winsor(X, scale_method='Standard', winsor=True)
