...
class summary():
    ...
    def train(self, splited_data):        
        ...
        if self.model_name == "linear": # False: 
            ...
            test_hat, train_hat, config = dl.dl_tuning_pipeline(splited_data, self.n_trials, self.tune)
    ...
    def store_result():
        ...
        if self.model_name == "neural network":
            model = f"{self.model_name} with number of trials {self.n_trials}"
        ...
        if self.train_size == 5:
            ...
            with open(
                f"result/{industry}/train_size_year_{model_config['train_size']}/{self.model_name}.pk", "wb") as f:
                    pickle.dump, f)