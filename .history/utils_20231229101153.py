...
def plot_model_result(industry, model, model_config):
    ...
    # transcation fee
    performance_rolling["performance"] -= 4
    ...
    # cumulative asset for port and market
    ...
    print(pd.DataFrame(market_cum_asset).set_index(market_rolling["ymd"]))
    print(pd.DataFrame(portfolio_cum_asset).set_index(performance_rolling["ymd"]))
