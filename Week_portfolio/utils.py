...
class feature_engineer:
    ...
    """
    def __init__(self, print_fea = True):
        self.print_fea = print_fea
    ...
    def transform_standard_winsor(self, X, scale_method = None, winsor = None):
        ...
        '''
        if self.print_fea:
            print("-------- Standardise and Winsorise [first: train_X, second: test_X] ---------")
    ...
    def transform_poly(self, X):        
        if self.print_fea:
            print("-------- Polynomial  [first: train_X, second: test_X] ---------")
        # 1. poly features
    ...
    def selected_pearsonr(self, X, y):        
        if self.print_fea:
            print("-------- Selecte with Correlation  [first: train_X, second: test_X] ---------")
        # here for train X
        selected_col = []
        ...
        X = X[:, selected_col]
        if self.print_fea:
            print(f'selected col corr: {selected_col}')
            
        return X, selected_col
          
    def transform_pca(self, X, var=10):
        if self.print_fea:
            print("-------- PCA ---------")
        ...
        # if has high correlte variables, do pca
        while True:
            step = 5
            print(var)
    ...
    def selected_bic(self, X, y, selected_col = None):
        if self.print_fea:            
            print("-------- Select with BIC [first: train_X, second: test_X] ---------")
