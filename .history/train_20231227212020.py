...
class summary():
    ...
    def __init__(self, 
        ...
        if input_size is not None:
            ...
            self.dir = os.getcwd()
            self.trian_start = self.initialise()
            self.
    ...
    def initialise(self):
        self.train_start = pd.Timestamp\
            (year = self.test_start-self.train_size, month = 1, day = 1)
        self.test_end = self.test_start + pd.DateOffset(months=self.test_size)
        return train_start
        # self.test_start
    ...
    # train
    def train_lsdecision(self):        
        ############################################
        # For 1 single peoriod, 
        # train and decide the long and short boundary
        ############################################
        (X_train, y_train, X_test, y_test) = self.Xy()
        
        if self.model_name == "linear": # False: 
            test_hat, train_hat, test_loss = linear_regression(self.param, self.data, self.tune)
        elif self.model_name == "decision tree": # False: 
            test_hat, train_hat, test_loss = decision_tree(self.param, self.data, self.tune)
        elif self.model_name == "xgboost": # False: 
            test_hat, train_hat, test_loss = xgboost(self.param, self.data, self.tune)
        elif self.model_name == "random forest":
            test_hat, train_hat, test_loss = random_forest(self.param, self.data, self.tune)
        elif self.model_name == "svm": # False:
            test_hat, train_hat, test_loss = svm(self.param, self.data, self.tune)
        elif self.model_name == "elastic net":
            test_hat, train_hat, test_loss = elastic_net(self.param, self.data, self.tune)
        elif self.model_name == "neural network":
            dl = deep_learning(self.industry, self.param, self.input_size)
            test_hat, train_hat, test_loss = dl.dl_tuning_pipeline(self.data, self.n_trials, self.tune)
        
        return train_hat, test_hat, test_loss

    def ls_decision(y_test, train_hat, test_hat):
