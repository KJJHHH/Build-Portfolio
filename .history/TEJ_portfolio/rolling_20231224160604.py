...
class Rolling_Prediction:
    ...
    def transform_decide_poly_kernel_predict(self, train_X, train_y, test_X, test_y, mode):
        ###########################################
        # 1. transform: kernel
        # 2. dicide if sort industry when predicting
        ###########################################
            ...
            # fit model
            if self.model == 'ols':
                fit_reg = fit_linear(train_X, train_y)
            elif self.model == 'wls':
    ...
    def predicting(self):
        ...
        if sort_industry == True:
            ...
            for industry in self.train["Industry_Eng"].unique():
                ...
                predict = self.decide_poly_kernel_predict(
                    train_X, train_y, 
                    test_X, test_y, self.mode)
        ...
        # if not sort industry
            ...
            if (good_data['len test'] != 0) and (good_data['enough sample'] == True):
                ...
                # get prediction
                predict = self.decide_poly_kernel_predict(train_X, train_y, test_X, test_y, self.mode)
