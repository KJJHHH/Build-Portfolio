...
class summary():
    ...
    def train_lsdecision(self):        
        ...
        if self.model_name == "linear": # False: 
            ...
            test_hat, train_hat, test_lloss = decision_tree(self.param, self.data, self.tune)
        elif self.model_name == "xgboost": # False: 
            test_hat, train_hat, test_lloss = xgboost(self.param, self.data, self.tune)
        elif self.model_name == "random forest":
            test_hat, train_hat, loss = random_forest(self.param, self.data, self.tune)
        elif self.model_name == "svm": # False:
            test_hat, train_hat, loss = svm(self.param, self.data, self.tune)
        elif self.model_name == "elastic net":
