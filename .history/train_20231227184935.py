...
class summary():
    ...
    def __init__(self, 
        ...
        if input_size is not None:
            ...
            self.param = self.params_setting(self.model_name)
    ...
    def traintest_update(self): 
        self.train_start = self.train_start + pd.DateOffset(months=test_size)
        self.test_start = self.test_start + pd.DateOffset(months=test_size)
        test_end = test_end + pd.DateOffset(months=test_size)
    
    # X_train, test, y_train, test
    def Xy(self, data, train_start, test_start, test_end): 
        temp_d = copy.copy(data)
        d_train = temp_d.reset_index()[
            (temp_d.reset_index()["ymd"] >= train_start) & \
            (temp_d.reset_index()["ymd"] <= test_start)]\
            .set_index(["code", "ymd"])
