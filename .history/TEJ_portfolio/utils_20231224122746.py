...
class transform_preprocess:
    def __init__(self, scale_method = None, winsor = True):
        '''
    ...
    def to_final_index(self, data, final):
        return data.reset_index().set_index(['coid', 'mdate'])
    ...
    def transform(self, X_train, X_test = None):
        ...
        if self.scale_method == 'Standard':
            ...
            X_train = self.to_final_index(X_train)
            if type(X_test) == pd.core.frame.DataFrame:
                X_test = X_test.groupby(['mdate']).apply(self.standard)
                X_test = self.to_final_index(X_test)
