...
class summary():
    ...
    def ls_decide(self, train_hat, test_hat):
        ...
        if (ls_bound == None) or (self.ls_decision[1] != "running"):   
            ls_bound = {}
            ls_bound["long"] = np.percentile(ls_decision_by, self.long_bound) 
            ls_bound["short"] = np.percentile(ls_decision_by, self.short_bound) 
            data_tp["per_long"] = np.percentile(ls_decision_by, self.long_bound) 
            data_tp["per_short"] = np.percentile(ls_decision_by, self.short_bound) 
            return data_tp, ls_bound
        
        # Running mean for the thresholds
        else: 
            ls_bound["long"] =  (ls_bound["long"] + 
                                        np.percentile(ls_decision_by, self.long_bound))/2
            ls_bound["short"] = (ls_bound["short"] +
                                        np.percentile(ls_decision_by, self.short_bound))/2
            data_tp["per_long"] = ls_bound["long"]
            data_tp["per_short"] = ls_bound["short"]
            return data_tp, ls_bound
    ...
    def compute_ret(self, data_tp, print_detail = True): # class and returns
        ...
        if self.do_short == True:
            single_date["pred_ls"] = np.where(ingle_date["prediction"] > single_date["per_long"], 1,
