...
def update_rolling_data(startfirst_rolling = False):
        # Update data for each rollin prediction

        if first_rolling == False:
            start_train_date += pd.DateOffset(weeks = 1)

        end_train_date = start_train_date + pd.DateOffset(weeks = self.train_size)
