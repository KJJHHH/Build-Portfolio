...
class deep_learning():
    ...
    def dl_tuning_pipeline(self, 
        ...
        try:
            print("===> tune start")
            trial = self.tune(X_train, y_train, n_trials)        
        ...
        data = (X_train, y_train, X_test,torch.tensor(np.array(y_test), dtype = torch.float32).to(device))
        test_hat, train_hat, test_loss = self.tune_train(self.config, data)
        test_hat = pd.DataFrame(test_hat, columns=["prediction"]).set_index(y_test.index)
        return test_hat, train_hat, self.config
