...
class Rolling_Prediction:
    def __init__(self, 
                data_week_return,  # data_process
                model, 
                mode):
        self.data_week_return = data_week_return
        self.model = model
        self.mode = mode
        self.start_train_date = min(data_week_return["mdate"])
        self.train_size = mode['train_size']

        # Initialise train, test data
        self.train, self.test, self.start_train_date, self.start_test_date = \
            update_data(data_week_return, mode['start_train_date'], mode['train_size'])
        self.
        self.train_X, self.train_y, self.test_X, self.test_y = \
            transform_data(self.train, self.test, mode)
        self.predict = predict_decide_poly_kernel(self.train_X, self.train_y, self.test_X, self.test_y, mode)
        self.check_data = check_data(self.train, self.test)
        self.result = result(self.predict, self.test_y, self.mode)
