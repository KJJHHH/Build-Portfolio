...
class Rolling_Prediction:
    ...
    def feature_engineering(self, X_train, X_test, y_train):
        ...
        # 2. poly 
        X_poly = self.feature_eng.transform_poly(X_scale_winsor, y_train)

        # 2. poly selected pearsonr
        X_selected_corr, selected_col_corr = self.feature_eng.transform_poly_selected_pearsonr(X_poly, y_train)
        X_test = self.feature_eng.transform_poly_selected_pearsonr(X_test, None, selected_col_corr)

        # 3. pca
        X_pca, X_test = self.feature_eng.transform_pca(Xselected_corr, X_test, var = 100)
