...
class summary():
    ...
    def Xy(self, data, train_start, train_end, test_end, n_component): 
    ...
    def train_lsdecision(
        ...
        if model_strategy["model"] == "linear": # False: 
            pred, y_train_hat, loss = linear_regression()
        elif model_strategy["model"] == "decision tree": # False: 
            test_hat, train_hat, loss = decision_tree()
            ...
            if tune == True:
                print(f"tuning {model_strategy['model']}")
                grid_s = GridSearchCV(estimator=RandomForestRegressor(), param_grid=self.param, cv = 5)
                grid_s.fit(np.array(X_train), y_train)
                print("Finish Tuning")
                best_params = grid_s.best_params_
            ...
            if tune == True:
                print(f"tuning {model_strategy['model']}")
                grid_s = GridSearchCV(estimator=SVR(), param_grid=self.param, cv = 5)
                grid_s.fit(np.array(X_train), y_train)
                print("Finish Tuning")
                best_params = grid_s.best_params_
            ...
            pred, y_train_hat, loss = elastic_net(self.param, fnl_df, tune)
        ...
        data_tp = pd.concat([y_test, test], axis = 1)  
        if model_strategy["ls_decision"][0] == "train":
            ls_decision = y_train_hat 
        else:
            ls_decision = pred
        ...
        elif (longshort_thres != None) and (model_strategy["ls_decision"][1] == "running"): 
