...
class transform_preprocess:
    def __init__(self, scale_method = 'Standart', winsor = True):
        '''
    ...
    def winsorise(self, data):
        for col in data.columns:
            data[col] = winsorize(data[col], limits=[0.001, 0.001], inplace=False)             
        return data
    
    def transform(self, X_train, X_test = None, scale_method = 'Standard', winsor = False):
        '''
        X_train
        X_test = None, X_test
        ...
        # scaling   
        X_train.reset_index().set_index('coid')
        if type(X_test) == pd.core.frame.DataFrame:
            X_test.reset_index().set_index('coid')

        if scale_method == 'Standard':
            X_train = X_train.groupby(['mdate']).apply(self.standard)
            if type(X_test) == pd.core.frame.DataFrame:
                X_test = X_test.groupby(['mdate']).apply(self.standard)
        elif scale_method == 'MinMax':
            X_train = X_train.groupby(['mdate']).apply(self.minmax)
            if type(X_test) == pd.core.frame.DataFrame:
                X_test = X_test.groupby(['mdate']).apply(self.minmax)
        ...
        if winsor == True:
            X_train = X_train.groupby(['mdate']).apply(self.winsorise)
            if type(X_test) == pd.core.frame.DataFrame:
                X_test = X_test.groupby(['mdate']).apply(self.winsorise)

        # reset index
        X_train = X_train.reset_index().set_index(['coid', 'mdate'])
        if type(X_test) == pd.core.frame.DataFrame:
            X_test = X_test.reset_index().set_index(['coid', 'mdate'])
...
def split_X_y(data):    
    # Split data
    try:
        data = data.set_index(["coid", "mdate"], inplace = True).copy()
    except:
        pass
    finally:
