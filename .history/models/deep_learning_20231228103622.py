...
class deep_learning():
    def __init__(self, industry, param, input_size):
    ...
    def dl_tuning_pipeline(self, 
        ...
        if tune == True:
            ...
            trial = self.tune(torch.tensor(np.array(X_train), dtype = torch.float32).to(device),
                torch.tensor(np.array(y_train), dtype = torch.float32).to(device), n_trials)
            config = trial.params
            print(config)
            
            # store and load config: tuned each 6 month                
            config_ = [config]
            try:
                with open(f"Hyperparam_nn/{self.industry}/nnconfig_{n_trials}trial.pickle", "rb") as f: 
                    config_past = pickle.load(f)
                    config_ = config_past + config_
            except:
                pass
            with open(f"Hyperparam_nn/{self.industry}/nnconfig_{n_trials}trial.pickle", "wb") as f: 
                pickle.dump(config_, f)
        else: # no tune
            with open(f"Hyperparam_nn/{self.industry}/nnconfig_{n_trials}trial.pickle", "rb") as f: 
                config_past = pickle.load(f)
            config = config_past[-1]
        ...
        test_hat, train_hat, test_loss = self.tune_train(config, data)
        test_hat = pd.DataFrame(test_hat, columns=["prediction"]).set_index(y_test.index)
        return test_hat, train_hat, test_loss
    ...
    # Functions for tune
    def get_mnist(self):
        with open("temp_data/temp_X", "rb") as f:
            X_train = pickle.load(f)
        with open("temp_data/temp_y", "rb") as f:
    ...
    def tune(self, X_train, y_train, n_try = 10):
        with open("temp_data/temp_X", "wb") as f:
            pickle.dump(X_train, f)
        with open("temp_data/temp_y", "wb") as f:
