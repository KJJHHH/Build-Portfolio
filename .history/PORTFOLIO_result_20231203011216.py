...
market_rolling["return"] = market_rolling["return"]*10
plt.plot(market_rolling.set_index("ymd")*100, label = "market")
# Shade the region where market_rolling > 0
plt.fill_between(market_rolling["ymd"], 0, market_rolling["return"],
                where=(market_rolling["return"] > 0), facecolor = "red", color='red', alpha=0.3, label='market > 0')
plt.legend()
plt.show()