...
plt.plot(market_rolling.set_index("ymd")*100, label = "market")
# Shade the region where market_rolling > 0
print(market_rolling["return"].values.flatten())
plt.fill_between(market_rolling["return"], 0, market_rolling["return"].values.flatten(),
                where=(market_rolling["return"] > 0).values.flatten(), color='red', alpha=0.3, label='market > 0')
plt.legend()
plt.show()