...
# ------------------------
from model import Data, Net, Net_tune, Net_tuned
...
def tune(X_train, y_train, n_try = 10):
    ...
    study.optimize(objective, n_trials=n_try, timeout=600000)
...
class summary():
    def __init__(self, dl_model = None):
        ...
        if dl_model is not None:
            ...
            self.batch_size = 25
            self.dir = os.getcwd()
            self.n_train_examples = batch_size * 30
            self.n_val_examples = batch_size * 10
    ...
    def data_train(self, fnl_df, model_strategy, param, longshort_thres = None):        
        ...
        if model_strategy["model"] == "linear": # False: 
            ...
            trial = tune(torch.tensor(np.array(X_train), dtype = torch.float32).to(device),
                torch.tensor(np.array(y_train), dtype = torch.float32).to(device), 10)
            config = trial.params
            print(config)
            # train
            ...
            y_hat, y_train_hat, loss = tune_train(config, data)
            #######################################
            #######################################
            # No tune
    ...
    def train_NN(self, inputs, labels, inputs_test, labels_test, lr, epoch):
        ...
        train_loader = DataLoader(dataset, batch_size=256, shuffle=False, drop_last=False)
