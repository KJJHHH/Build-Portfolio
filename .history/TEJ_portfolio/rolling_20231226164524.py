...
class Rolling_Prediction:
    ...
    def featureeng_kernel_addconstant(self, train_X, train_y, test_X, test_y):
        ###########################################
        ...
        # Fea engineering 
        # 1. scale and winsro
        X_scale_winsor = self.feature_eng.transform_standard_winsor(train_X, scale_method='Standard', winsor=True)
        X_test = self.feature_eng.transform_standard_winsor(test_X, scale_method='Standard', winsor=True)
        ...
        # 3. selected pearsonr
        X_selected_corr, selected_col_corr = self.feature_eng.selected_pearsonr(X_poly, train_y)
        X_test = X_test[selected_col_corr]

        # 4. pca
        # Too small size in testdata. 
        # Combine train and test data
        train_size = X_selected_corr.shape[0]
        print(X_selected_corr.shape, X_test.shape)
        Xs = np.concatenate((X_selected_corr, X_test), axis=0)
        Xs = self.feature_eng.transform_pca(
            Xs, 
            var = min(X_selected_corr.shape[0] - 1, int(X_selected_corr.shape[1]/10)))
        X_pca, X_test = Xs[:train_size], Xs[train_size:]
        X_train = X_pca
        ...
        """
        train_X = pd.DataFrame(train_X).set_index(train_y.index)
        test_X = pd.DataFrame(train_X).set_index(test_y.index)
        ...
        if sort_industry == True:
            ...
            for industry in self.train["Industry_Eng"].unique():
                ...
                train_X, train_y, test_X, test_y = \
                    self.featureeng_kernel_addconstant(train_X, train_y, test_X, test_y)
